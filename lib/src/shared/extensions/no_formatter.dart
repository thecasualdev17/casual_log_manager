...
extension NoFormatter on Log {
  ...
  static Log fromNoFormatString(
    ...
    return Log(
      ...
      logLevel: LogLevel.fromString(parts.length > 1 ? parts[1] : ''),
      message: parts.length > 2 ? parts[2] : '',
      label: parts.length > 3 ? parts[3] : '',
      stackTrace: parts.length > 4 && parts[4].isNotEmpty
          ? StackTrace.fromString(parts[4])
          : null,
    );
  }
}
